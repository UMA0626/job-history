...
- 自社製フレームワークの習得  
  現行の開発言語は`Ruby on Rails`だったが、面倒を見れるメンバーがチームから減ってきたこともあり自社製フレームワーク(`Java`ベース)に書き換えた  
...
## 自走力、キャッチアップ力
A社にいた頃は開発業務は未経験の状態であり、プライベートではPythonを少し程度しか触っていなかったため、B社に入社後の`Java`, `JavaScript`/`TypeScript`、`Angular`、`Vue`、`Nuxt3`などはすべて未経験から習得する必要がありました。  
そのような状態からすぐ自社製フレームワークを使った開発をする必要がありましたが、積極的に質問したりソースコードをできる限り自分で読んでみることで1ヶ月程度でキャッチアップでき、基本的な画面であれば1人で実装できるようになりました。  
これを成功体験として内面化し、以降のプロジェクトでも未経験の技術に対して恐れることなく積極的に学習し、提案に組み込んでいくことができました。

## アーキテクト的な役割の経験
OJTの段階から開発支援としてCIの整備や開発用テンプレート生成`Gradle`タスクを作成するなど開発環境を整備したり、  
B社での案件では「どういった技術を使い、どのAWSサービスをどう構成して要件を満たすか」について検討・提案を行う経験をしました。  
また、B社の2024年4月～の案件では「他サービス開発時に利用できるコードベース」を用意するため、どのような技術を採用するかであったり、仮に別の技術等を採用することになっても可能な限り差し替えがしやすいように設計するといった点に気をつけて検討を進めました。  
上記により、ただコーディングするだけではなく「他の人が開発・保守するときにやりやすいようにする」という意識・視点を養えました。  
...
## 顧客対応
A社ではフロントSEとしてお客様との定例打ち合わせなどに参加しており、  
B社でも開発メンバーとして要件定義や仕様の調整などを行っておりました。  
雑談やアイスブレイクはあまり得意ではないですが、誠実に対応することを心がけており、分からないことも煙に巻いたりせず素直に言ったりお客様と一緒に考えていくことでお客様からも信頼・評価を頂けました。



# 希望するキャリアビジョン
大まかなキャリアプランとしては、管理職ではなく所謂スペシャリストを目指していきたいと考えております。  

その上で、第一にユーザー目線での開発をしたいと考えております。  
現職も受託開発なので直接のお客様目線に立つことはある程度可能ですが、  
私の場合は担当している2案件がどちらも「お客様のユーザー」(所謂BtoBtoCのC)にサービスを提供する案件であり、  
お客様のユーザーとの接点が乏しくお客様からヒアリングしても又聞きだったりお客様自身もそこまで詳しくなかったりすることもあり、  
サービスの本来のユーザー目線に立ちきれず、歯がゆい思いをすることもありました。  
そのため、本当にターゲットとしているユーザー目線での開発ができる環境に身を置きたいと思っています。  
